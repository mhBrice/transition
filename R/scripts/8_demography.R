...
# most abundant species in each group
...
### Combute basal area per hectare
...
### RECRUITS ####
tree_data <- tree_data %>% 
  mutate(recru = ifelse(ETAT %in% c(40:48), 1, 0)) %>% 
  mutate(recru_ba = recru*DHP)
...
recru_ba <- reshape2::dcast(tree_data,
                            ...
                            fill = 0,
                            value.var = "recru_ba") %>%
  select(ID_PE, ID_PE_MES, year_measured, all_of(MySpecies))

### DEAD ####

# Some dead trees have DHP mesures others don't so we need to look a the previous measure

tree_data <- tree_data %>% 
  mutate(dead = ifelse(state=="dead", 1, 0)) %>%
  mutate(dead_ba = dead*DHP)
...
tree_data <- tree_data %>% 
  mutate(harv = ifelse(state=="harvested", 1, 0)) %>% 
  mutate(harv_ba = harv*DHP)
...
par(mar = c(2.5, 8, .2, .2))
image(tab[, 28:1], col = (pal), axes = FALSE)
