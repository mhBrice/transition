...
plot_score <- function(score_ls, ylab = "Score",
  ...
  ord <- order(colMeans(score_ls[["cv_msm_glb"]]), decreasing = TRUE)
  ord_mod <- order(unlist(lapply(score_ls, mean)), decreasing = TRUE)
  ...
  lgd = legend("topright", legend = mod_names[ord_mod],
             pch = 21, col = col_mod[ord_mod],
             pt.bg = alpha(col_mod[ord_mod],.5), pt.cex = .9,
             lwd = lwd, cex = .75, bg = alpha("white", .8),
